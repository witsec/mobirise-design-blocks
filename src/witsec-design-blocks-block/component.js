mbrApp.loadComponents(
	...
	{"witsec-design-blocks-block":{
			...
			// If the witsecBlockURL is set AND the block is set to be activated, something must've gone wrong activating the block. Let's try again :)
			_onLoadFromProject: function(b,	a) {
				if (this._params.witsecBlockURL && this._params.witsecBlockActivate) {
					setTimeout(function() {
						mbrApp.alertDlg("A Design Block hasn't been activated properly. Please activate it manually using the gear icon.");
					}, 1000);
				}
			},
			...
			_params:{
				witsecGallery:       {type:"separator",title:"<a href='#' class='btn btn-primary witsec-show-design-gallery'>OPEN GALLERY</a>"},
				witsecBlockURL:      {type:"text",title:"Block URL (paste here)",default:""},
				witsecBlockActivate: {type:"switch",title:"Activate block",default:!1,condition:["witsecBlockURL"]}
			},

			// Default message we show the user
			_message: "Use the gear icon to access the Design Blocks Gallery or paste the location of your own block.",

			_onParamsChange:function($item, param, val){
				if (param == "witsecBlockActivate" && val == true) {
					// We add a random number to the URL, to avoid any caching
					var witsecBlockURL = this._params.witsecBlockURL + "?" + Math.floor(Math.random() * 100000000);

					// Try to grab the block
					var request = $.ajax({ url: witsecBlockURL, dataType: "text" });
					...
					request.success(function(result) {
						...
						if (error) {
							mbrApp.alertDlg("<b>Invalid block</b><br /><br />" + error);
							return false;
						}
